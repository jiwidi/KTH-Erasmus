...
tuple<int,double> HMM::nextObservation(const vector<int>& obs){
    int state=this->calculateState(obs);
    vector<double> tone=A.getvector(state);
    Matrix Btrans=B.transpose();
    vector<double> au=Btrans*tone;

    int ind=0;
    // confidence TODO
    double max=0;
    double sum=0;
    for (int m=0;m<au.size();m++){
        if (au[m]>max){
            max=au[m];
            ind=m;
            sum+=au[m];
    ...
    tuple<int,double> result(ind,max);
    tuple<int,double> re(2,0.4);
