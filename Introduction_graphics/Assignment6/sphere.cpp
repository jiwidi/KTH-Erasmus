...
bool Sphere::closestIntersection(const Ray& ray, double maxLambda,
    ...
    // • std::shared ptr<const Renderable> renderable: Reference in the form of a smart pointer to the Object
    // with which the intersection occurred. Use the term shared_from_this() here.
    // • const real lambda: Ray parameter where λ ∈ [0, maxLambda].
    // • const Vec3d &normal: constant reference to the normal direction at the point of intersection on the
    // Object’s surface.
